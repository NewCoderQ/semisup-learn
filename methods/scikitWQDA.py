...
class WQDA(BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=[]):
        ...
        Parameters
        ----------
        trian vector(n_samples, n_features)
        X : array-like, shape = [n_samples, n_features]
            Training vector, where n_samples in the number of samples and
            n_features is the number of features.

        train label(integers)(n_samples)
        ...
        """        
        K = len(set(y))        # get the number of classes
        # instances_number < features_number
        if X.shape[0] < X.shape[1]: # less instances than dimensions -> use shrinkage
            self.use_shrinkage = True
        
        # split the instances according to the class_label
        # [1 2 4 3 0 2 1 2 3]
        # [array([4], dtype=int64), array([0, 6], dtype=int64), array([1, 5, 7], dtype=int64), 
        # array([3, 8], dtype=int64), array([2], dtype=int64)]
        kindices = [numpy.where(y==k)[0] for k in range(K)]
        if len(sample_weight) == 0:         # no provided weight
            # Count number of occurrences of each value in array of non-negative ints.
            qsum = numpy.bincount(y.astype(int))
            # init the weights as element one
            sample_weight = numpy.ones(len(y))
        else:
            # calculate the sum of sample_weight
            qsum = numpy.array([numpy.sum(sample_weight[kindices[k]]) for k in range(K)])
        # reshape the sample_weight
        sample_weight = numpy.reshape(sample_weight, (len(sample_weight),1))
        # calculate the priors_ of each class
        ...
        for k in range(K):      # for each class
            ...
            if len(self.covariance_) == 0:      # the initial covariance_
